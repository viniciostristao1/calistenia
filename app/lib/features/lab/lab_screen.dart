      ...
      child: ExpansionTile(
        ...
        children: [
          _slider(
            ...
            0.25,
            3,
            '${_params.speed.toStringAsFixed(2)}×',
          ...
          _slider(
            ...
            0,
            2,
            _params.intensity.toStringAsFixed(2),
          ...
          _slider(
            ...
            0.5,
            2,
            '${_params.scale.toStringAsFixed(2)}×',
            (v) => setState(() => _params = _params.copyWith(scale: v)),
          ),
          _slider(
            'Partículas',
            _params.particleCount.toDouble(),
            0,
            80,
            '${_params.particleCount}',
            (v) => setState(
              () => _params = _params.copyWith(particleCount: v.round()),
            ),
          ),
          _slider(
            'Duração',
            _params.duration.inMilliseconds.toDouble(),
            200,
            3000,
          ...
          _slider(
            'Atraso',
            _params.delay.inMilliseconds.toDouble(),
            0,
            2000,
            '${_params.delay.inMilliseconds} ms',
            (v) => setState(
              () => _params = _params.copyWith(
                delay: Duration(milliseconds: v.round()),
              ),
            ),
          ),
          _slider(
            'Repetir',
            _params.repeat.toDouble(),
            0,
            5,
            _params.loopForever ? '∞ (loop)' : '${_params.repeat}×',
            (v) =>
                setState(() => _params = _params.copyWith(repeat: v.round())),
          ),
          _slider(
            'Valor (+XP / ⭐)',
            _valor,
          ...
          _slider(
            ...
            0,
            100,
            _params.valor2 <= 0 ? 'off' : '+${_params.valor2.round()}',
            (v) => setState(() => _params = _params.copyWith(valor2: v)),
          ),
        ],
      ),
    ),
    ...
    child: Column(
      ...
      children: [
        Row(
          ...
          children: [
            Text(nome, style: TextStyle(color: AppColors.text, fontSize: 13.5)),
            Text(
              rotulo,
              style: TextStyle(
