...
atual = **v0.20.0** (release com APKs por arquitetura). **Nome de exibição = "Calis
...
player + edição manual tocando no dia.
...
texto (`Clipboard`); **peso** por exercício (`Exercicio.pesoKg`); **cor** por exercício
(pontinho, 10 cores, `corIndex`); barras da Progressão alinhadas na base.
...
quantas fez). Modelo `RegistroProgressao` (liga por nome) + `progressaoProvider`
(`progressao_v1`). Gráfico com widgets puros (sem pacote de chart).
